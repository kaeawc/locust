...
class HttpSession(requests.Session):
    ...
    def request(self, method, url, name=None, catch_response=False, **kwargs) -> Response:
        ...
        request return a context manager to work as argument to a with statement. This will
        allow the request to be marked as a fail based on the content of the response, even if
        the response code is ok (2xx). The opposite also works, one can use catch_response to
        catch a request and then mark it as successful even if the response code
        was not (i.e 500 or 404).
        """
